...
class TestInvertFrankfurterRates:
    ...
    def test_inverts_typical_response(self):
        # Frankfurter: 1 ILS = 0.25 USD, 1 ILS = 0.20 EUR
        # Our store: 1 USD = 4 ILS, 1 EUR = 5 ILS
        payload = {
            "amount": 1.0,
            "base": "ILS",
            "date": "2026-05-29",
            "rates": {"USD": 0.25, "EUR": 0.20},
        }
        result = _invert_frankfurter_rates(payload)
        assert result is not None
        usd_to_ils, eur_to_ils = result
    ...
    def test_returns_none_for_missing_currency(self):
        assert _invert_frankfurter_rates({"rates": {"USD": 0.25}}) is None

    def test_returns_none_for_zero_rate(self):
        """A zero rate would cause a division-by-zero and corrupt the DB row."""
        assert _invert_frankfurter_rates({"rates": {"USD": 0, "EUR": 0.20}}) is None
