...
Note: **#14** (`generate_batch_number` race — two concurrent receipts computing the same next sequence) was deliberately skipped in this pass. A correct fix needs either a Postgres advisory lock or a catch-and-retry-on-`IntegrityError` loop; both add real complexity and the retry approach isn't safely testable against the SQLite test backend the way the `with_for_update()` fixes elsewhere were. Left open for a dedicated pass.
